...
description: Resolve git merge conflicts with AI-powered analysis, structural merging via mergiraf, lock file handling, and stacked PR duplicate detection. Use when a rebase, merge, cherry-pick, or revert has conflicts.
...
This skill works best with [mergiraf](https://mergiraf.org/) installed and configured as a git merge driver. Run `MergeButler setup` to install and configure everything automatically.

Without mergiraf, files that would benefit from structural merging fall through to AI analysis, which still works but is less precise for structural changes.

## Your Task

### Step 1: Detect Context

Determine the current platform and run the appropriate status script.

On Unix/macOS (or Git Bash on Windows):

```bash
bash .github/skills/resolve-conflicts/scripts/conflict-status.sh
```

On Windows (PowerShell):

```powershell
.github/skills/resolve-conflicts/scripts/conflict-status.ps1
```
...
```bash
bash .github/skills/resolve-conflicts/scripts/categorize-conflicts.sh
```

On Windows (PowerShell):

```powershell
.github/skills/resolve-conflicts/scripts/categorize-conflicts.ps1
```
...
Accept theirs to clear the conflict markers. The content doesn't matter since Step 3 regenerates lock files from the resolved dependency manifest:
...
Run mergiraf as a second pass (it may have already run as a merge driver during the git operation, but sometimes conflicts remain):

```bash
mergiraf solve -- <file> --compact --keep-backup=false
```

After running mergiraf, read the file and check for remaining conflict markers (`<<<<<<<`). If markers remain, proceed with AI analysis (see Step 2c).
